# Text-Based Testing for vtio

This directory contains text-based tests for the vtio terminal input parser,
...
- **Key events**: `key(press:ctrl-alt-shift-a)` format
  ...
  - Special states like `:keypad`, `:caps_lock`, `:num_lock` are appended if present
  - Base layout key shown as `:base=<key>` if present
  - Associated text shown as `:text="<text>"` if present
